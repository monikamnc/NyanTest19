...
const red = [255, 0, 0];
const pink = [255, 0, 100];
const blue = [0, 0, 255];
const orange = [200, 150, 50];

//senseLeds.Leds.clear();

//matrix.clear();
const { nya, mazePoints, cross, lel } = (() => {
	const _ = blue;
	const X = red;
    const P = pink;
    const O = orange;
	...
	const mazePoints = lodash.map(nya, (maze) => {
		return lodash.flatMap(maze, (pixel, pos) => {
			if(pixel === X) {
				return [[ pos % WIDTH, Math.floor(pos / WIDTH) ]];
			}
	...
	return { nya, mazePoints, cross, lel }
})();
...
const displayNya = () => {
	senseLeds.setPixels(nya);
};

const displayLel = () => {
	senseLeds.setPixels(lel);
};
